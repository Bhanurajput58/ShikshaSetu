                ...
                <div className="apply-status-section">
                  ...
                  <div className="apply-status-indicator">
                    {myRequest.status === 'pending' ? 'Application Pending' :
                      myRequest.status === 'accepted' ? 'Application Accepted' :
                      myRequest.status === 'scheduled' ? 'Mentorship Scheduled' :
                      myRequest.status === 'cancelled' ? 'Mentorship Cancelled' :
                      myRequest.status === 'rejected' ? 'Application Rejected' :
                      myRequest.status}
                    {myRequest.status === 'cancelled' && myRequest.cancelReason && (
                      <div style={{ fontSize: '0.8em', color: '#666', marginTop: '4px' }}>
                        Reason: {myRequest.cancelReason}
                      </div>
                    )}
                  </div>
                </div>
        ...
        >
          <Card className="mentorship-card">
            <CardContent className="card-content">
              ...
              {myRequest && myRequest.educator && myRequest.status !== 'cancelled' ? (
                    ...
                    <div className="mentor-info">
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                        <div>
                          ...
                          {myRequest.status === 'scheduled' && myRequest.scheduledDate && (
                            <Typography variant="caption" color="primary" sx={{ display: 'block', mt: 0.5 }}>
                              Scheduled for {new Date(myRequest.scheduledDate).toLocaleDateString()}
                            </Typography>
                          )}
                        </div>
                        <Button
                ...
                <div className="empty-state">
                  ...
                  <Typography className="empty-text">
                    {myRequest && myRequest.status === 'cancelled' ? 'Mentorship Cancelled' : 'No mentor assigned yet'}
                  </Typography>
                  <Typography className="empty-subtext">
                    {myRequest && myRequest.status === 'cancelled' 
                      ? `Your mentorship was cancelled. ${myRequest.cancelReason ? `Reason: ${myRequest.cancelReason}` : ''}`
